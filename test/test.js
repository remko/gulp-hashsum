...
describe("gulp-hashsum", function() {
  ...
  it("should write hashes to current dir", function(done) {
    ...
    stream.on("end", function() {
      var data = fs
        .readFileSync(path.resolve(process.cwd(), "SHA1SUMS"))
        .toString();
      expect(data).to.match(
        /3ff1f9baca7bf41fe4a12222436025c036ba88bf {2}(.*)\n14de86e007f14bc0c6bc9a84d21cb9da908495ae {2}(.*)\n/
      );
      done();
    });
  ...
  it("should not accept streams", function(done) {
    ...
    var file = new Vinyl({
      contents: fs.createReadStream("/dir1/file1"),
      path: "/dir1/file1"
    });
    expect(function() {
      stream.write(file);
  ...
  it("should generate JSON when option is set", function(done) {
    ...
    stream.on("end", function() {
      var data = fs
        .readFileSync(path.resolve(process.cwd(), "/dir1/SHA1SUMS"))
        .toString();
      var json;
      try {
      ...
      expect(json).to.deep.equal({
        file1: "3ff1f9baca7bf41fe4a12222436025c036ba88bf",
        file2: "14de86e007f14bc0c6bc9a84d21cb9da908495ae"
      });
      done();
    });
